let handler = async (m, { conn, usedPrefix, command, text }) => {
  ...
  // Cek apakah ada mention atau nama
  let target = m.mentionedJid?.[0] || m.quoted?.sender || null;
  let name;

  if (target) {
    // Normalisasi JID dulu (hindari LID)
    const jid = conn.getJid ? conn.getJid(target) : conn.decodeJid(target);
    name = await conn.getName(jid);
    
    // Kalau masih LID atau angka, ambil dari text yang diinput
    if (jid.endsWith('@lid') || /^\d+$/.test(name)) {
      name = text || 'User';
    }
  } else {
    name = text;
  ...
  if (!name) return m.reply(`Masukkan nama atau mention user!\n\nContoh:\n${usedPrefix + command} Abay gay\n${usedPrefix + command} @user`);

  let data = conn.khodam[name] || pickRandom(khodamList);
  conn.khodam[name] = data;
...
handler.help = ['cekkhodam <nama> atau @user'];
handler.tags = ['fun'];
