const handler = async (m, { conn, isOwner, isAdmin, isBotAdmin }) => {
  if (!(isAdmin || isOwner)) return global.dfail('admin', m, conn);
  if (!isBotAdmin)           return global.dfail('botAdmin', m, conn);
  const users = m.mentionedJid?.length ? m.mentionedJid : (m.quoted ? [m.quoted.sender] : []);
  if (!users.length) return m.reply('Tag/quote user yang mau didemote!');
  for (const u of users) await conn.groupParticipantsUpdate(m.chat, [u], 'demote');
  await m.reply(`✅ Berhasil demote ${users.length} user`);
};
handler.help     = ['demote @user'];
handler.tags     = ['group'];
handler.command  = /^(demote|deadmin)$/i;
handler.group    = true;
handler.botAdmin = true;
handler.description = "Menurunkan admin grup menjadi member biasa melalui mention atau reply.";

export default handler;
